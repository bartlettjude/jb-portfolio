...
export function Footer() {
  ...
  return (
    <footer className="relative border-t border-[var(--border)] bg-[var(--background)]">
      {/* Subtle gradient */}
      <div 
        className="pointer-events-none absolute inset-0 bg-gradient-to-t from-[var(--muted)]/30 to-transparent"
        aria-hidden="true"
      />
        ...
        <div className="flex flex-col gap-8 sm:flex-row sm:items-end sm:justify-between">
          ...
          <div className="flex flex-col items-start gap-4 sm:items-end">
            <div className="flex items-center gap-6">
              {siteConfig.socials.github && (
                <a
                  ...
                  className="group flex items-center gap-2 text-sm font-medium text-[var(--text-secondary)] transition-colors duration-200 hover:text-[var(--accent)]"
                  target="_blank"
                  rel="noreferrer"
                >
            ...
            <p className="text-xs text-[var(--text-secondary)]">
              © {year} {siteConfig.name}. Crafted with intention.
            </p>
          </div>
        </div>
      </Container>
